...
{
    public class CoalMineTests
    {
        [Fact]
        public void TestHourlyProductionRate()
        {
            ...
            Assert.True(produced, "Production should succeed.");
            decimal coalStock = mine.GetProductQuantity("Węgiel");
            Assert.True(coalStock == 102, $"Expected 102 tons after 1 hour, got {coalStock}");
            Assert.True(Math.Abs(mine.PreciseStorage - 102.0) < 0.0001, $"Expected precise storage ~102.0, got {mine.PreciseStorage}");
        }

        [Fact]
        public void TestWeeklyStorageBottleneck()
        {
            ...
            Assert.True(Math.Abs(mine.PreciseStorage - 38356.0) < 0.001, $"Expected capped storage at 38356.0, got {mine.PreciseStorage}");
            decimal coalStock = mine.GetProductQuantity("Węgiel");
            Assert.True(coalStock == 38356m, $"Expected integer warehouse stock 38356, got {coalStock}");

            // The 169th hour should trigger halt
            bool producedAfterHalt = mine.ProduceHourly(company, day: 8, hour: 1);
            Assert.False(producedAfterHalt, "Production should halt when storage is full.");
            Assert.True(haltEventTriggered, "Halt event should have been triggered.");
        }

        [Fact]
        public void TestCoalSalesPrice()
        {
            ...
            Assert.True(sold, "Selling coal should succeed.");
            decimal expectedRevenue = 50 * 400m;
            decimal balanceDiff = company.Balance - initialBalance;
            Assert.True(balanceDiff == expectedRevenue, $"Expected revenue {expectedRevenue}, got {balanceDiff}");
            Assert.True(mine.GetProductQuantity("Węgiel") == 52m, $"Expected remaining warehouse stock 52, got {mine.GetProductQuantity("Węgiel")}");
            ...
            Assert.True(Math.Abs(mine.PreciseStorage - 154.0) < 0.001, $"Expected precise storage synced and increased to ~154.0, got {mine.PreciseStorage}");
        }

        [Fact]
        public void TestEmployeeAndLevelScaling()
        {
            var company = new Company("TestCorp", 1000000m);
            var mine = new CoalMine("Kopalnia A");
            company.Buildings.Add(mine);
            ...
            Assert.True(mine.MaxEmployees == 1200, "Level 1 max employees should be 1200");
            mine.CurrentEmployees = 1500; // clamp to 1200
            Assert.True(mine.CurrentEmployees == 1200, "Current employees should clamp to max");

            // Level 2: Max employees = 3000
            mine.Level = 2;
            Assert.True(mine.MaxEmployees == 3000, "Level 2 max employees should be 3000");
            mine.CurrentEmployees = 2500;
            Assert.True(mine.CurrentEmployees == 2500, "Current employees should be 2500");

            // Technology multiplier
            mine.TechnologyMultiplier = 1.5f;
            // Production: 2500 * 0.085 * 1.2 * 1.5 = 382.5 tons/hour
            mine.ProduceHourly(company, 1, 9);
            Assert.True(Math.Abs(mine.PreciseStorage - 382.5) < 0.001, $"Expected 382.5, got {mine.PreciseStorage}");
        }
    }
}
