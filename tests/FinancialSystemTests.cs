...
namespace Conglomerate.Financials.Tests
{
    ...
    {
        [Fact]
        public void TestPnLCalculations()
        {
            ...
            Assert.True(pnlMid.Revenue == 50000m, $"Revenue: oczekiwano 50000, otrzymano {pnlMid.Revenue}");
            Assert.True(pnlMid.RawMaterials == -10000m, $"COGS: oczekiwano -10000, otrzymano {pnlMid.RawMaterials}");
            Assert.True(pnlMid.GrossProfit == 40000m, $"GrossProfit: oczekiwano 40000, otrzymano {pnlMid.GrossProfit}");
            Assert.True(pnlMid.EBITDA == 25000m, $"EBITDA: oczekiwano 25000, otrzymano {pnlMid.EBITDA}");
            Assert.True(pnlMid.Depreciation == -1000m, $"Depreciation: oczekiwano -1000, otrzymano {pnlMid.Depreciation}");
            Assert.True(pnlMid.EBIT == 24000m, $"EBIT: oczekiwano 24000, otrzymano {pnlMid.EBIT}");
            ...
            Assert.True(engine.CurrentMonthIndex == 2, "CurrentMonthIndex powinien wynosić 2");
            Assert.True(engine.MonthlyHistory.Count == 1, "Historia powinna zawierać 1 wpis");

            var archivedSnap = engine.MonthlyHistory[0];
            Assert.True(archivedSnap.PnL.CorporateTax == -4560m, $"Tax: oczekiwano -4560, otrzymano {archivedSnap.PnL.CorporateTax}");
            Assert.True(archivedSnap.PnL.NetIncome == 19440m, $"NetIncome: oczekiwano 19440, otrzymano {archivedSnap.PnL.NetIncome}");
            Assert.True(engine.RetainedEarnings == 19440m, $"RetainedEarnings: oczekiwano 19440, otrzymano {engine.RetainedEarnings}");
        }

        [Fact]
        public void TestBalanceSheetEquation()
        {
            // 1. Inicjalizacja
            var engine = new FinancialEngine(startingCash: 100000m, startingShareCapital: 100000m);
            var bs1 = engine.CalculateCurrentBalanceSheet();
            Assert.True(bs1.IsBalanced, "Początkowy bilans musi być zrównoważony");
            Assert.True(bs1.TotalAssets == 100000m);

            // 2. Zaciągnięcie kredytu (Liabilities +50k)
            engine.AdjustLoans(50000m);
            var bs2 = engine.CalculateCurrentBalanceSheet();
            Assert.True(bs2.IsBalanced, "Bilans po kredycie musi być zrównoważony");
            Assert.True(bs2.Loans == 50000m);
            Assert.True(bs2.Cash == 150000m);
            Assert.True(bs2.TotalAssets == 150000m);
            ...
            Assert.True(bs3.IsBalanced, "Bilans po zakupie fabryki musi być zrównoważony");
            Assert.True(bs3.Cash == 30000m);
            Assert.True(bs3.PropertyBookValue == 120000m);
            Assert.True(bs3.TotalAssets == 150000m);
            ...
            Assert.True(bs4.IsBalanced, "Bilans po zamknięciu miesiąca musi być zrównoważony");
            Assert.True(bs4.RetainedEarnings == 35640m, $"RetainedEarnings: oczekiwano 35640, otrzymano {bs4.RetainedEarnings}");
            Assert.True(bs4.PropertyBookValue == 119000m, $"PropertyBookValue: oczekiwano 119000, otrzymano {bs4.PropertyBookValue}");
            Assert.True(bs4.Cash == 66640m, $"Cash: oczekiwano 66640, otrzymano {bs4.Cash}"); // 30k starting + 50k sales - 5k salaries - 8.36k tax = 66.64k
        }

        [Fact]
        public void TestHistoryAndQuarterlyRoll()
        {
            var engine = new FinancialEngine(startingCash: 100000m, startingShareCapital: 100000m);
            ...
            Assert.True(engine.MonthlyHistory.Count == 6, $"Miesięczna historia: {engine.MonthlyHistory.Count}");
            Assert.True(engine.QuarterlyHistory.Count == 2, $"Kwartalna historia: {engine.QuarterlyHistory.Count}");

            // Sprawdzenie wartości pierwszego kwartału (Miesiące 1, 2, 3)
            var q1 = engine.QuarterlyHistory[0];
            Assert.True(q1.PeriodName == "Kwartał 1");
            Assert.True(q1.PnL.Revenue == 60000m, $"Q1 Revenue: {q1.PnL.Revenue}");
            Assert.True(q1.PnL.Salaries == -15000m, $"Q1 Salaries: {q1.PnL.Salaries}");
            Assert.True(q1.PnL.Depreciation == -1500m, $"Q1 Depreciation: {q1.PnL.Depreciation}");
            Assert.True(q1.PnL.CorporateTax == -8265m, $"Q1 Tax: {q1.PnL.CorporateTax}"); // 2755 * 3 = 8265
            Assert.True(q1.PnL.NetIncome == 35235m, $"Q1 NetIncome: {q1.PnL.NetIncome}"); // 11745 * 3 = 35235
            ...
            Assert.True(engine.MonthlyHistory.Count == 24, "Miesięczna historia powinna być obcięta do maksymalnie 24 miesięcy");
            // Pierwszy zachowany element to miesiąc 3 (indeks okresu = 3) bo miesiące 1 i 2 zostały usunięte
            Assert.True(engine.MonthlyHistory.First().PeriodIndex == 3, $"Pierwszy okres w historii: {engine.MonthlyHistory.First().PeriodIndex}");
        }

        [Fact]
        public void TestSaveAndLoadSettingsAndMetadata()
        {
            ...
            Assert.True(settings.StartingCash == 75000m);
            Assert.True(settings.GlobalCorporateTax == 0.25m);
            ...
            Assert.True(testFile != null, "Plik zapisu powinien istnieć");

            var meta = SaveGameManager.GetSaveMetadata(testFile.FullName);
            Assert.True(meta.CorporationName == "TestSaveCorp", $"Oczekiwano TestSaveCorp, otrzymano {meta.CorporationName}");
            Assert.True(meta.CurrentDay == 5, $"Oczekiwano Day 5, otrzymano {meta.CurrentDay}");
            Assert.True(meta.CurrentHour == 12, $"Oczekiwano Hour 12, otrzymano {meta.CurrentHour}");
            Assert.True(meta.NetWorth == 75000m, $"Oczekiwano NetWorth 75000, otrzymano {meta.NetWorth}");
            ...
            Assert.True(container.State.CompanyName == "TestSaveCorp");
            Assert.True(container.State.Cash == 65000m); // 75000 - 10000
            Assert.True(container.State.TaxRate == 0.25m);
            Assert.True(container.State.Buildings.Count == 1);

            var bData = container.State.Buildings[0];
            Assert.True(bData.Name == "Farma Testowa");
            Assert.True(bData.X == 2);
            Assert.True(bData.Y == 3);
        ...
        [Fact]
        public void TestWarehouseResourceTransferAndPersistence()
        {
            ...
            Assert.True(farm.GetProductQuantity("Mleko") == 0m, $"Farm milk: {farm.GetProductQuantity("Mleko")}");
            Assert.True(farm.GetProductQuantity("Mięso") == 0m, $"Farm meat: {farm.GetProductQuantity("Mięso")}");
            Assert.True(foodWh.GetProductQuantity("Mleko") == 18m, $"FoodWh milk: {foodWh.GetProductQuantity("Mleko")}");
            Assert.True(foodWh.GetProductQuantity("Mięso") == 12m, $"FoodWh meat: {foodWh.GetProductQuantity("Mięso")}");

            // Węgiel do magazynu kopalnianego (30 Coal przeniesione dobowo, 102 Coal wyprodukowane godzinowo po transferze)
            Assert.True(mine.GetProductQuantity("Węgiel") == 102m, $"Mine coal: {mine.GetProductQuantity("Węgiel")}");
            Assert.True(miningWh.GetProductQuantity("Węgiel") == 30m, $"MiningWh coal: {miningWh.GetProductQuantity("Węgiel")}");
            ...
            var saveFiles = SaveGameManager.GetSaveFiles();
            var testFile = saveFiles.FirstOrDefault(f => f.Name.Contains(testSaveName));
            Assert.True(testFile != null);

            var container = SaveGameManager.LoadGame(testFile.FullName);
            Assert.True(container != null);
            Assert.True(container.State.Buildings.Count == 5, $"Buildings: {container.State.Buildings.Count}");
            var loadedFoodWh = container.State.Buildings.FirstOrDefault(b => b.Type == "FoodWarehouse");
            Assert.True(loadedFoodWh != null);
            Assert.True(loadedFoodWh.Name == "Magazyn Spozywczy");
            Assert.True(loadedFoodWh.X == 4);
            Assert.True(loadedFoodWh.Y == 3);

            var loadedMlekoItem = loadedFoodWh.Warehouse.FirstOrDefault(i => i.Key == "Mleko");
            Assert.True(loadedMlekoItem.Batch != null);
            Assert.True(loadedMlekoItem.Batch.Quantity == 18m);
        ...
        [Fact]
        public void TestFacilityPnLTracking()
        {
            ...
            Assert.True(initialCostPnL == -10000m, $"Initial cost PnL: oczekiwano -10000, otrzymano {initialCostPnL}");

            farm.AddProduct("Mleko", 10);
            bool sold = farm.SellResource("Mleko", 5, company, 1, 9);
            Assert.True(sold, "Sprzedaż mleka powinna się udać");
            ...
            Assert.True(currentPnL == expectedPnLAfterSale, $"PnL po sprzedaży: oczekiwano {expectedPnLAfterSale}, otrzymano {currentPnL}");
            ...
            Assert.True(finalPnL == expectedPnLAfterMaintenance, $"PnL po utrzymaniu: oczekiwano {expectedPnLAfterMaintenance}, otrzymano {finalPnL}");
        }

        [Fact]
        public void TestLogisticsAndOPEX()
        {
            ...
            // Początkowy stan
            Assert.True(gameManager.Logistics.ActiveTrips.Count == 0, "Brak aktywnych podróży na starcie");
            Assert.True(warehouse.GetProductQuantity("Mleko") == 0m, "Brak towaru u odbiorcy");
            ...
            Assert.True(gameManager.Logistics.ActiveTrips.Count == 1, "Jedna aktywna podróż po 6 godzinach");
            var trip = gameManager.Logistics.ActiveTrips[0];
            Assert.True(trip.CargoAmount == 10, "Załadowano 10 sztuk");
            Assert.True(trip.RemainingHours == 2, "Czas podróży to 2 godziny");
            ...
            Assert.True(lastTransaction != null, "Powinna istnieć transakcja transportowa");
            Assert.True(lastTransaction.Amount == -40m, "Koszt transportu (OPEX) wynosi 40 zł");

            // Towar powinien zniknąć ze źródła
            Assert.True(farm.GetProductQuantity("Mleko") == 40m, "W farmie powinno zostać 40 sztuk mleka");
            ...
            Assert.True(gameManager.Logistics.ActiveTrips.Count == 0, "Podróż zakończona");
            Assert.True(warehouse.GetProductQuantity("Mleko") == 10m, "Towar dostarczony do magazynu");
        }
    }
}
